...
function make_inventory {
   ...
   count=1 #이 인자는 각 노드 그룹의 노드 수를 뜻한다. 처음엔 1로 명시하여 노드 수만큼 늘어나고 노드 그룹이 바뀌면(ex: boot->master) 다시 0으로 돌아간다.
      ...
      if [ ${#arr_node_name[${count}-1]} -eq 0 ] #만약 인자가 더이상 존재하지 않는다면(노드 명의 길이가 null이면)
      then
   ...
   if [ ${arr[${arr_count}]} = "end" ] #만약 arr(노드명을 명시한 배열) 에서 end라는 이름이 명시되면 모든 노드에 대한 이어쓰기가 마무리 됬으므로 반복문을 종료한다.
   then
   break
   else
   continue
